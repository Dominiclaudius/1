...
class PrintParamsEveryNStepsHook(tf.train.SessionRunHook):  # 注意这里是 tf.train
    def __init__(self, print_every_n_steps=1):
    ...
    def after_run(self, run_context, run_values):
        ...
        if global_step % self.print_every_n_steps == 0:
            total_params = 0
            ...
            print("[Step %d] Total trainable parameters: %d" % (global_step, total_params))
FLAGS = flags.FLAGS

# Settings for logging.

flags.DEFINE_string('logdir', r'E:\mobile\mobile-deeplab-v3-plus-master\log',
...
flags.DEFINE_float('base_learning_rate', 1e-2,
...
flags.DEFINE_multi_integer('model_input_size', [256, 256],
...
# Dataset settings.
flags.DEFINE_string('dataset_name', 'people_segmentation',
...
flags.DEFINE_string('dataset_dir', r'E:\mobile\mobile-deeplab-v3-plus-master\datasets\tfrecord', 'Where the dataset reside.')

# run settings.
flags.DEFINE_boolean('debug', False, 'Debug or not')

flags.DEFINE_string('backbone_type', "MobilenetV3", '[MobilenetV2|MobilenetV3]')
...
flags.DEFINE_string('export_dir', r"E:\mobile\mobile-deeplab-v3-plus-master\result", 'Directory to export model.')
...
def train():
    ...
    print_params_hook = PrintParamsEveryNStepsHook(print_every_n_steps=1)
    train_spec = tf.estimator.TrainSpec(
        ...
        hooks=[time_hist],)
      # hooks=[time_hist, print_params_hook],)
