...
def squeeze_and_excite(x, reduction=4, weight_decay=0.00004):
    ...
    print(f">>> SE block used | Input channel: {in_channel} <<<")
    scale = global_pool(x)
    in_channel = x.get_shape().as_list()[-1]
...
# def eca_module(x, k_size=3, weight_decay=0.00004):
#     in_channel = x.get_shape().as_list()[-1]
#     print(f">>> ECA block used | Input channel: {in_channel} <<<")
...
#     )(squeeze)  # [B, C, 1]
#     excitation = tf.nn.sigmoid(conv)
#     excitation = tf.transpose(excitation, [0, 2, 1])
